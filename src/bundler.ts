...
export class Bundler {
  ...
  private async bundle(
    ...
    const importsPromises = Helpers.getAllMatches(content, IMPORT_PATTERN).map(
      async match => {
    ...
    for (const imp of imports) {
      ...
      if (!imp.found) {
        ...
        // and bundle it
        const bundledImport = await this.bundle(
          imp.fullPath,
          impContent,
          dedupeFiles,
          includePaths,
        ...
        if (
          this.usedImports != null &&
          this.usedImports[imp.fullPath] == null
        ) {
      ...
      if (imp.ignored) {
        ...
        if (contentToReplace == null) {
          ...
          contentToReplace = `/*** IMPORTED FILE NOT FOUND ***/${os.EOL}${
            imp.importString
          }/*** --- ***/`;
        }
  ...
  private async resolveImport(importData, includePaths): Promise<any> {
    try {
      ...
      const dirname = path.dirname(importData.fullPath);
      const basename = path.basename(importData.fullPath);

      const cssBasename =
        basename.substr(0, basename.lastIndexOf(".")) + ".css";
      const cssFilePath = path.join(dirname, cssBasename);
      ...
      try {
        ...
        try {
          const underscoredFilePath = path.join(dirname, `_${basename}`);
          ...
          if (includePaths.length) {
            ...
            importData.fullPath = path.resolve(
              includePaths[0],
              importData.path
            );
